#!/usr/bin/env bash
# install-codex.sh — install trainlint into OpenAI Codex CLI.
#
...
#   1. merge trainlint's hooks (with absolute paths baked in) into ~/.codex/hooks.json
#   2. render commands/*.md into ~/.codex/prompts/trainlint-<name>.md  (Codex slash commands)
#
# Idempotent: re-running replaces trainlint's own entries, never duplicates them, and
# never touches other hooks you have. Re-run after moving the repo (paths are baked).
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
[ -f "$ROOT/hooks/router.py" ] || { echo "error: $ROOT/hooks/router.py not found — run this from the trainlint dir" >&2; exit 1; }
...
# --- 2. prompts: commands/*.md -> ~/.codex/prompts/trainlint-<name>.md ------------------
for f in "$ROOT"/commands/*.md; do
  name="$(basename "$f" .md)"
  out="$PROMPTS/trainlint-$name.md"
  sed "s|\${CLAUDE_PLUGIN_ROOT}|$ROOT|g" "$f" > "$out"
  echo "  prompt -> $out   (/trainlint-$name)"
done

echo
echo "trainlint installed for Codex at $CODEX_HOME"
echo "  root: $ROOT"
echo "  commands: /trainlint-init /trainlint-plan /trainlint-quiz /trainlint-viz /trainlint-lint"
echo "Start a new Codex session to load the hooks. Verify with: codex  -> /trainlint-lint"
