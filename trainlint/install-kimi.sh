#!/usr/bin/env bash
# install-kimi.sh — install trainlint into the Kimi CLI (Kimi Code).
#
...
#   1. merges trainlint's hooks (abs paths + TRAINLINT_HOST=kimi baked in) into the
#      `hooks = [...]` array in ~/.kimi/config.toml, preserving your other hooks
#   2. installs the 5 commands as Kimi skills (~/.kimi/skills/trainlint-*/SKILL.md)
#      and registers that dir in `extra_skill_dirs`
# Idempotent (re-run replaces only trainlint's entries) and non-destructive. Re-run
...
merged_hooks = existing + trainlint_hooks
...
print(f"  hooks  -> {config}  ({len(trainlint_hooks)} trainlint + {len(existing)} kept)")
...
for f in "$ROOT"/commands/*.md; do
  name="$(basename "$f" .md)"
  dir="$SKILLS/trainlint-$name"
  mkdir -p "$dir"
  {
    echo "---"
    echo "name: trainlint-$name"
    # carry the command's own description line if present
    grep -m1 '^description:' "$f" || echo "description: trainlint $name"
    echo "---"
  ...
  echo "  skill  -> $dir/SKILL.md   (/skill:trainlint-$name)"
done

echo
echo "trainlint installed for Kimi at $KIMI_DIR"
...
echo "Start a fresh Kimi session to load the hooks. Commands: /skill:trainlint-plan etc."
