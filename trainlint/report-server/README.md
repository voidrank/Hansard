# trainlint report UI — live multi-tenant report viewer

Turns each operator's **local** trainlint reports into a page teammates can open at
...
- `relay_internal_secret.txt` (0600) — the `x-trainlint-relay` header the backend trusts as admin
...
1. Run trainlint normally — `viz.py` renders and `serve.py` auto-starts the backend + relay. The relay
...
- ⚠️ **Plugin upgrades overwrite the marketplace tree.** Keep a copy of every edit in
  `data_root/viz_patches/` and reapply after upgrading (see the memory note on trainlint customizations).
