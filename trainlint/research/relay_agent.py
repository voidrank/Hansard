...
x-trainlint-relay: <secret> (data_root/relay_internal_secret.txt, auto-generated 0600) — the backend
...
def _relay_url():
    return os.environ.get("TRAINLINT_RELAY_URL", DEFAULT_URL).strip() or DEFAULT_URL
...
def _token():
    ...
    tok = os.environ.get("TRAINLINT_REPORT_TOKEN", "").strip()
    if tok.lower() == "none":
        return ""
...
def _local_request(method, path, body_b64, ct):
    ...
    headers = {"x-trainlint-relay": get_or_create_secret()}
    if ct:
...
def share(email):
    ...
    req = urllib.request.Request(
        ...
        headers={"Authorization": f"Bearer {token}", "Content-Type": "application/json",
                 "User-Agent": "Mozilla/5.0 (trainlint-relay-agent)"},  # bare urllib UA gets CF bot-blocked (1010)
        method="POST")
    try:
